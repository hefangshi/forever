  ...
  if (format) {
    ...
    procs.forEach(function (proc) {
      rows.push(mapColumns('[' + index + ']', function (column) {
        return forever.columns[column] ? forever.columns[column].get(proc) : 'MISSING';
      }));
...
forever.logEvents = function (monitor) {
  ...
  monitor.on('restart', function () {
    forever.out.info('Script ' + monitor.data.file + ' pid:' + monitor.child.pid + ' restart attempt #' + monitor.times);
  });

  monitor.on('start', function () {
    forever.out.info('Script ' + monitor.data.file + ' start');
  });

  monitor.on('stop', function () {
    forever.out.info('Script ' + monitor.data.file + ' stoped');
  });

  monitor.on('exit:code', function (code, signal) {
    forever.out.error('Forever detected pid: ' + monitor.child.pid + ' script: ' + monitor.data.file + ((code !== null && code !== undefined) ? ' exited with code: ' + code : ' was killed by signal: ' + signal));
  });
};
